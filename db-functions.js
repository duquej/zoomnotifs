...
function retrieveMissingLinks(classes) {
  return new Promise(async (resolve, reject) => {
    ...
    const coursePromises = classes.map((classObj) =>
      linksRef.doc(classObj.course).get()
    );
    Promise.all(coursePromises)
      .then((res) => {
        ...
        const nonLinkedClasses = classes.filter(
          (classObj) =>
            !classesMap[classObj.course] ||
            !classesMap[classObj.course][classObj.section]
        );
          ...
          for (let i = 0; i < courseInfo["days"].length; i += 1) {
            ...
            if (currentTime.length > 0) {
              for (let j = 0; j < currentDayGroup.length; j += 1) {
                ...
                promises.push(
                  insertData(collectionsRef, docName, email, docData)
                );
              }
            }
          }
            ...
            .then((querySnapshotForStudents) => {
              querySnapshotForStudents.forEach(async (student) => {
                ...
                if (student.id === email) {
                  ...
                  if (
                    docData.course === classCode &&
                    docData.section === classSection
                  ) {
