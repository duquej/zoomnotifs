...
function parseSchedule(id) {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < scheduleClasses.length; i++) {
      ...
      const courseCode = $(scheduleClass)
        .children("span")
        .first()
        .text()
        .trim();
    ...
    for (let i = 0; i < expander.length; i++) {
      ...
      if (coursesMap[courseCode]) {
        ...
        // Grab section ids
        const coursePinnedSection = $(courseHeader.parent.parent)
          .find(".ng-binding.classnbr-pinned")
          .toArray();
        classInfo["section"] = coursePinnedSection.map((section) =>
          section.children[0].data.trim()
        );

        // Grab days for specific sections
        const coursePinnedDays = $(courseHeader.parent.parent)
          .find(".mtg-pat.classnbr-pinned")
          .toArray();
        classInfo["days"] = coursePinnedDays.map((day) =>
          $(day.children[2]).text().trim()
        );

        // Grab times for specific sections
        const coursePinnedTimes = $(courseHeader.parent.parent)
          .find(".mtg-time.classnbr-pinned")
          .toArray();
        classInfo["times"] = coursePinnedTimes.map((time) =>
          $(time.children[2]).text().trim()
        );
