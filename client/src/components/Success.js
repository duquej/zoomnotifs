...
export function Success(props) {
    ...
    if (linksArray.length > 0) {
      ...
      linksArray.forEach(([className, meetingID]) => {
        ...
        if (meetingID.length > 0) {
          ...
          const zoomLink = `https://cornell.zoom.us/j/${meetingID.replace(
            /-/g,
            ""
          )}`;
          requestPromises.push(
            axios.get(
              `/api/addLink?course=${course}&section=${section}&link=${zoomLink}`
            )
          );
        }
      });
      ...
      Promise.all(requestPromises)
        .then((res) => {
          ...
          props.hideSuccess();
          Message.success(
            `Successfully added links to ${requestPromises.length} ${
              requestPromises.length === 1 ? "class" : "classes"
            }`
          );
          // Clear links
          setLinks({});
  ...
  return (
    <Modal
      ...
      title={
        <span>
          <CheckCircleTwoTone twoToneColor="#52c41a" /> Successfully signed up
          with {props.email}
        </span>
      }
      closable={false}
    ...
    >
      <Form form={form} {...completeLayout} name="addLinks" onFinish={addLinks}>
        <Form.Item>
          We identified classes in your schedule that we don't currently have
          Zoom links for. Save your Zoom meeting IDs so we can send them to you
          before classes!
        </Form.Item>
        <Form.Item>
          <Tooltip
            title={
              <span>
                Access your class on{" "}
                <a href="https://canvas.cornell.edu/" target="_blank">
                  Canvas
                </a>{" "}
                > Zoom > Meeting ID (e.g. 982-252-173)
              </span>
            }
          >
            ...
            >
              <Input
                ...
                onChange={(e) =>
                  setLinks({
                    ...links,
                    [`${classInfo.course}-${classInfo.section}`]: e.target
                      .value,
                  })
                }
              />
